...
const bot = new Telegraf("6201071990:AAEMJd0CjnoCVJU6zLVY1nXtuv1zL73tDNk");
const fs = require('fs');
    ...
    try {
        ...
        if (currentHour === +Fajr.slice(0, 2) && currentMinute === +Fajr.slice(3)) {
            users.forEach(async user => {
                ...
                if (FajrTime) {
                    let Id = user.user
                    try{
                        await bot.telegram.sendMessage(Id, adahnTime("اذان الفجر")[`${Math.floor(Math.random() * 3) + 1}`]).catch(() => { return })
        ...
        if (currentHour === +Dhuhr.slice(0, 2) && currentMinute === +Dhuhr.slice(3)) {
            users.forEach(async user => {
                ...
                if (DhuhrTime) {
                    let Id = user.user
                    await bot.telegram.sendMessage(Id, adahnTime("اذان الظهر")[`${Math.floor(Math.random() * 3) + 1}`]).catch(() => { return })
                }
            })
        }
        //  وقت اذان العصر
        if (currentHour === +Asr.slice(0, 2) && currentMinute === +Asr.slice(3)) {
            users.forEach(async user => {
                ...
                if (AsrTime) {
                    let Id = user.user
                    await bot.telegram.sendMessage(Id, adahnTime("اذان العصر")[`${Math.floor(Math.random() * 3) + 1}`]).catch(() => { return })
                }
            })
        }
        //  وقت اذان المغرب
        if (currentHour === +Maghrib.slice(0, 2) && currentMinute === +Maghrib.slice(3)) {
            users.forEach(async user => {
                ...
                if (MaghribTime) {
                    let Id = user.user
                    await bot.telegram.sendMessage(Id, adahnTime("اذان المغرب")[`${Math.floor(Math.random() * 3) + 1}`]).catch(() => { return })
                }
            })
        }
        //  وقت اذان العشاء
        if (currentHour === +Isha.slice(0, 2) && currentMinute === +Isha.slice(3)) {
            users.forEach(async user => {
                ...
                if (IshaTime) {
                    let Id = user.user
                    await bot.telegram.sendMessage(Id, adahnTime("اذان العشاء")[`${Math.floor(Math.random() * 3) + 1}`]).catch(() => { return })
                }
            })
        }
